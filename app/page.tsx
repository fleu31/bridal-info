...
export default async function Home(){
  ...
  const d = data ?? {
    ...
    heroButtons: [{label:'逆算チェッカー', url:'/tools/checker'},{label:'記事一覧', url:'/articles'},{label:'ディレクトリ', url:'/directory'}],
    timeline: [
      {title:'T−6〜3か月', points:['トーン/質感の土台づくり','強い施術は早期に試射','毎回の仕上げで安定化']},
      {title:'T−8〜4週',   points:['出力を控えめに','新規は避ける','前撮りが近い場合はP優先']},
      {title:'T−2〜1週',   points:['仕上げ（導入・鎮静）のみ','刺激は避ける','メイクとの相性を確認']},
      {title:'T−1〜3日',   points:['施術は行わない','睡眠・塩分・飲酒を整える','衣装や移動での擦れ対策']},
    ],
    concerns: [
      {tag:'赤み・くすみ', title:'トーンを整える', points:['早期に試射→反応を確認','直前は仕上げのみ','紫外線/睡眠/塩分の管理']},
      {tag:'毛穴・質感',   title:'凹凸の扱い方', points:['ピーク対策はT-4週以前','強い出力は早期に評価','メイクとの相性を確認']},
      {tag:'ニキビ・跡',   title:'炎症があるとき', points:['無理はしない','刺激の強い施術は見送り','医療機関の判断を優先']},
    ],
    disclaimer: '※本サイトは一般情報の提供を目的とします。具体的な適応判断や同意取得は医療機関で行われます。'
  }
      ...
      <section className="border-b">
        <div className="container py-16 grid md:grid-cols-2 gap-10 items-end">
          ...
          <div className="relative aspect-[4/3] rounded-md overflow-hidden">
            {d.heroImage ? (
              <Image
                src={urlFor(d.heroImage).width(1600).height(1200).url()}
                alt={d.heroImage.alt || 'ヒーロー画像'}
                fill
