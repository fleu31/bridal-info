...
export async function POST(req: NextRequest) {
  try {
    ...
    // --- Turnstile（設定されているときだけチェック、必ずcatch） ---
    if (TURNSTILE_SITE && TURNSTILE_SECRET) {
      try {
        ...
        // 検証サービスにつながらない等
        return NextResponse.json({ ok: false, error: 'captcha_verify_error' }, { status: 400 })
      }
    }

    // --- Sanity保存（失敗しても無視して続行） ---
    if (token && projectId) {
      try {
    ...
    if (RESEND_API_KEY && CONTACT_TO && CONTACT_FROM) {
      try {
        ...
        await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({ from: CONTACT_FROM, to: [CONTACT_TO], subject, html })
        })
      } catch (e) {
