...
export default function MiniChecker(){
  ...
  return (
    <div className="card mt-8">
      <div className="font-medium">ミニ逆算チェッカー</div>
      ...
      <div className="grid md:grid-cols-2 gap-6 text-sm mt-4">
        <div className="card"><div className="font-medium">今できること</div>
          <ul className="mt-2 list-disc list-inside text-neutral-700">{advice.can.map((x,i)=>(<li key={i}>{x}</li>))}</ul>
        </div>
        <div className="card"><div className="font-medium">避けたいこと</div>
          <ul className="mt-2 list-disc list-inside text-neutral-700">{advice.avoid.map((x,i)=>(<li key={i}>{x}</li>))}</ul>
        </div>
      </div>
      <div className="mt-3 text-xs text-neutral-500">※本ツールは一般情報の提供を目的とします。具体的な適応・出力の判断は医療機関で行われます。</div>
    </div>
  )
}
